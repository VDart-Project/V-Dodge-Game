...
hs_bg = pygame.image.load('galaxy.png')
hs_bgX = 0
...
playerimg = pygame.image.load('vdart_white.png')
playerImg = pygame.transform.scale(playerimg, (76, 39))
playerX = 80
playerY = 300
...
def show_score(x, y):
    print_score_game = score_font.render("Score: " + str(score * 10), True, (220, 0, 220))
    screen.blit(print_score_game, (x, y))
...
while running_game:
    ...
    while winning:
        ...
        if num_asteroid >= 11:
            asteroid(asteroidX[10], asteroidY[10])
        if num_asteroid >= 12:
            asteroid(asteroidX[11], asteroidY[11])
        if num_asteroid >= 13:
            asteroid(asteroidX[12], asteroidY[12])
        if num_asteroid >= 14:
            asteroid(asteroidX[13], asteroidY[13])
        if num_asteroid >= 15:
            asteroid(asteroidX[14], asteroidY[14])
        if num_asteroid >= 16:
            asteroid(asteroidX[15], asteroidY[15])
        if num_asteroid >= 17:
            asteroid(asteroidX[16], asteroidY[16])
        if num_asteroid >= 18:
            asteroid(asteroidX[17], asteroidY[17])
        if num_asteroid >= 19:
            asteroid(asteroidX[18], asteroidY[18])
        if num_asteroid >= 20:
            asteroid(asteroidX[19], asteroidY[19])
        pygame.display.update()
    ...
    screen.blit(bg1, (0, 0))
    screen.blit(go_font.render("GAME OVER", True, (255, 0, 0)), (195, 155))
    score_value = score_font.render("Score: " + str(score*10), True, (255, 100, 100))
    screen.blit(score_value, (315, 275))
    Next = pygame.key.get_pressed()
    print_next = r_font.render("PRESS SPACEBAR TO RETRY", True, (255, 255, 255))
    screen.blit(print_next, (245, 375))
